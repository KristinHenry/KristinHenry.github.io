...
d3.csv("randy_4_a.csv", function(error, data) {
  ...
  data.forEach(function(d){
   ...
   w.domain(d3.extent(data, function(d){ return d.views; }))
  ...
  cell.on("mouseover", function(d){
    ...
    g.append("g")
      ...
      .enter().append("rect")
      .attr("class", "bar")
      .attr("x", function(d){ return width- w(d.views); })
      .attr("y", function(d,i){ return y(d.date); }) //return 10*i; })
      .attr("width", function(d){ return w(d.views); })
      .attr("height", 1)
      .attr("opacity", .4)
      .style("fill", function(d){  console.log("fill bar", d); return d['song_parody'] == "y" ? "#c42edb" : "#30b6bf" })
